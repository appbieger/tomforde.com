...
async function optimizeImage(filePath) {
  ...
  try {
    ...
    if (!needsResize && originalSize < 500000) {
      ...
      console.log(`⊘ Übersprungen (bereits optimiert): ${path.basename(filePath)} (${formatBytes(originalSize)})`);
      stats.skipped++;
    ...
    if (optimizedSize < originalSize) {
      ...
      console.log(`  ${formatBytes(originalSize)} → ${formatBytes(optimizedSize)} (${percent}% kleiner)`);
    } else {
